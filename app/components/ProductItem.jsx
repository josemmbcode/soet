...
export default function ProductItem({ product }) {
  ...
  return (
    <div className=" m-6 overflow-hidden rounded-lg hover:shadow-slate-500 hover:shadow-md transition-shadow duration-200 sm:relative group">
      ...
      <div className="flex justify-between px-1 capitalize sm:border-b-2 sm:border-b-soet border-x-2 border-x-soet sm:rounded-b-lg overflow-hidden m-0">
        <p className="p-1 m-0 text-sm sm:text-base max-sm:h-14">{product.name}</p>
        <h2 className="font-bold text-sm sm:text-lg p-1">${product.price}</h2>
      </div>
